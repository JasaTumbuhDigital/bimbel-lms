...
export default async function AdminUsersPage({ searchParams }: Params) {
    ...
    // Jalankan query list user dan list class level secara paralel
    const [currentUSer, { users, total }, classLevels] = await Promise.all([
    ...
    return (
        <div className="min-h-screen bg-slate-50 text-slate-900 p-8">
            <div className="max-w-6xl mx-auto space-y-6">
                ...
                <header className="flex items-center justify-between border-b border-slate-200 pb-4">
                    <div>
                        ...
                        <p className="text-xs text-slate-600">
                            Kelola akun siswa, tutor, dan admin ({total} pengguna ditampilkan)
                        </p>
                    </div>
                ...
                {/* Tab Navigasi — menggunakan Link, bukan state, supaya URL bisa di-bookmark/share */}
                ...
                {/* Filter & Search (menggunakan form GET supaya URL berubah) */}
                <form method="GET" className="flex gap-3 flex-wrap">
                    ...
                    />
                    {/* Filter tingkatan kelas — hanya tampil di tab Siswa */}
                    {activeTab === "student" && (
                        <select
                            name="classLevelId"
