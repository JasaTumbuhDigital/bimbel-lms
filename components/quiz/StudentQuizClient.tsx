...
export default function StudentQuizClient({ quiz, courseId, nextLessonUrl }: { quiz: any, courseId: string, nextLessonUrl?: string | null }) {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8">
                    ...
                    {quiz.userProgress && (
                        <div className="flex items-center gap-2 border-l pl-4 ml-2">
                            ...
                            {quiz.userProgress.isPassed ? (
                                <span className="text-green-600 font-semibold">Lulus (Nilai: {Math.round(quiz.userProgress.bestScore)}%)</span>
                            ) : (
                                <span className="text-red-600 font-semibold">Belum Lulus (Nilai Terbaik: {Math.round(quiz.userProgress.bestScore)}%)</span>
                            )}
                        </div>
                    )}
                </div>
            ...
            {result ? (
                <div className={`p-8 rounded-xl border-2 text-center mb-8 animate-in fade-in zoom-in duration-300 ${result.passedThisAttempt ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                    ...
                    <p className="text-slate-700 mb-4">Nilai Akhir Anda: <span className="font-bold text-2xl ml-1">{Math.round(result.score)}%</span></p>
                    
                    <div className="flex flex-col sm:flex-row justify-center gap-4 mt-6">
                        {result.passedThisAttempt ? (
                            nextLessonUrl ? (
                                <Link href={nextLessonUrl} className="bg-purple-600 text-white px-6 py-2.5 rounded-lg font-medium hover:bg-purple-700 shadow-sm transition-colors">
                                    Materi Selanjutnya &rarr;
                                </Link>
                            ) : (
                                <Link href={`/student/courses/${courseId}`} className="bg-green-600 text-white px-6 py-2.5 rounded-lg font-medium hover:bg-green-700 shadow-sm transition-colors flex items-center gap-2">
