...
- **FR-10 (P1, T1):** Sistem harus mendukung upload dokumen materi (PDF, PPT) dengan preview langsung di browser tanpa perlu download.
  - Catatan implementasi: skema awal menggunakan `@cyntler/react-doc-viewer` untuk preview seluruh jenis dokumen. Jika ada masalah kompatibilitas, akan dievaluasi untuk diupgrade (opsional) di Fase selanjutnya.
