...
**Terkait dokumen:** PRD.md v2.0 (§5.1, §6, §7.1), SDD.md v1.1 (§3.1, §3.2, §4.2, §5, §6.1, §7.3, §8), Implementation Plan v2.2 (Fase 1)
**Status:** Draft — fitur pertama yang di-breakdown ke level TSD, mengikuti urutan Fase 1 Implementation Plan
...
- **Pembuatan akun tutor/admin** — secara teknis pola Server Action-nya identik dengan `createStudentAccount` (hanya beda `role` & tidak insert `student_profiles`), tapi UI-nya masuk cakupan TSD Admin Dashboard (Fase 5), bukan TSD ini. Skema fungsi disiapkan generik agar reusable (lihat §4.2).
- **Penetapan `class_level_id` default saat akun siswa dibuat** — logic detailnya didetailkan di TSD Class Level Module (Fase 1b); TSD ini hanya mendefinisikan bahwa `createStudentAccount` **memanggil** helper tersebut, bukan mengimplementasikannya ulang.
...
- `User.email` unik di level aplikasi (Prisma `@unique`) **dan** di Supabase `auth.users` — dua sumber kebenaran ini harus tetap sinkron; `createStudentAccount` wajib cek keduanya sebelum insert (lihat §4.2 edge case).
- `mustChangePassword` **hanya** relevan untuk `StudentProfile` sesuai keputusan SDD §3.3 — akun tutor/admin dibuat langsung oleh developer/owner saat setup awal instance (di luar UI Tier 1), bukan lewat alur admin-created seperti siswa, jadi tidak butuh flag ini.
...
Validasi password (`min 8, kombinasi huruf-angka`) mengikuti FR-1 secara verbatim — diterapkan konsisten di `createStudentAccount` (password sementara auto-generate juga wajib lolos aturan ini) dan `changePassword`.

---

## 4. Server Actions — Spesifikasi Detail

> Lokasi: `app/(auth)/**/actions.ts` untuk aksi yang dipicu dari halaman auth publik, `app/(admin)/users/actions.ts` untuk `createStudentAccount`. Semua Server Action mengembalikan bentuk hasil konsisten: `{ success: boolean; data?: T; error?: { field?: string; message: string } }`.
...
| Proses | 1. Validasi Zod → 2. `supabase.auth.signInWithPassword({ email, password })` → 3. Jika sukses, ambil `users.role` via Prisma berdasarkan `authId` dari session → 4. Redirect sesuai role: `student` → `/dashboard`, `tutor` → `/tutor/dashboard`, `admin` → `/admin/dashboard` |
| Output sukses | `{ success: true, data: { role: Role } }` |
...
| Input | `{ name: string; email: string; phone?: string; classLevelId?: string }` (`createStudentAccountSchema`) |
| Proses | 1. Validasi Zod → 2. Cek email di `users` (Prisma) **dan** cross-check tidak perlu ke `auth.users` langsung karena `users.email` adalah sumber kebenaran aplikasi yang selalu sinkron dengan insert di langkah 4 → 3. Jika email sudah ada → return error **sebelum** lanjut (lihat edge case FR-2) → 4. Generate password sementara (random, lolos aturan `changePasswordSchema.newPassword`, misal 10 karakter alfanumerik) → 5. `supabase.auth.admin.createUser({ email, password: tempPassword, email_confirm: true })` → 6. Insert `users` (role: `student`) + `student_profiles` (`classLevelId` = input atau hasil `getDefaultClassLevel()` dari Class Level Module, `mustChangePassword: true`) dalam **satu transaction Prisma** (`prisma.$transaction`) → 7. Return kredensial sementara ke UI admin (email + password) untuk dikirim manual |
...
| `users` | `update_own` | `auth.uid() = auth_id` — user hanya bisa update row miliknya sendiri (dipakai `updateProfile`); insert/delete row `users` **tidak** lewat RLS client-side sama sekali — hanya lewat `createStudentAccount` yang jalan di server dengan `service_role` key (bypass RLS by design, karena butuh insert atas nama user lain) |
...
| `app/(auth)/change-password-required/page.tsx` | Halaman paksa ganti password (redirect otomatis jika `mustChangePassword = true`) | Siswa yang baru login pertama kali |
...
| `app/(admin)/users/new/page.tsx` | Form `createStudentAccount` | Admin only |
...
| `EMAIL_ALREADY_EXISTS` | "Email sudah digunakan." | `createStudentAccount`, realtime saat blur field email |
...
- [ ] Admin membuat akun siswa dengan email baru → sukses, `student_profiles` ter-insert dengan `must_change_password = true`
- [ ] Admin membuat akun siswa dengan email yang sudah ada → error muncul **saat blur**, bukan hanya setelah submit (edge case FR-2)
...
- [ ] Siswa login pertama kali → dipaksa ke halaman ganti password, tidak bisa mengakses halaman lain sebelum ganti password selesai
- [ ] Siswa ganti password dari Settings dengan `currentPassword` salah → ditolak
...
- **Diperlukan oleh:** Hampir semua TSD berikutnya (Class Level, Course, Quiz, dst) bergantung pada middleware & session dari fitur ini — TSD ini secara sengaja dikerjakan lebih dulu (lihat catatan pembuka dokumen).
- **Soft dependency:** `createStudentAccount` memanggil `getDefaultClassLevel()` dari Class Level Module (Fase 1b) — jika Class Level Module belum ada saat Auth Module mulai dikerjakan, gunakan stub sementara (`classLevelId` wajib diisi manual di form, bukan default otomatis) agar Fase 1 tidak ter-blok menunggu Fase 1b selesai penuh.
